...
static struct bt_work_q conn_tx_workq;
...
static void tx_complete_work(struct bt_work *work);
...
#define CONN_UPDATE_TIMEOUT  OS_MSEC(CONFIG_BT_CONN_PARAM_UPDATE_TIMEOUT)

static void deferred_work(struct bt_work *work);
...
bt_slist_t bt_auth_info_cbs = BT_SLIST_STATIC_INIT(&bt_auth_info_cbs);
#endif /* CONFIG_BT_SMP || CONFIG_BT_CLASSIC */

#if defined(CONFIG_BT_CONN_DYNAMIC_CALLBACKS)
static bt_slist_t conn_cbs = BT_SLIST_STATIC_INIT(&conn_cbs);

#define BT_CONN_CB_DYNAMIC_FOREACH(_cn) \
	for (struct bt_conn_cb *_cn = BT_SLIST_PEEK_HEAD_CONTAINER(&conn_cbs, _cn, _node); \
	     _cn != NULL; \
	     _cn = BT_SLIST_PEEK_NEXT_CONTAINER(_cn, _node))
#else
#define BT_CONN_CB_DYNAMIC_FOREACH(_cn) \
...
#if defined(CONFIG_BT_CONN_TX)
static void frag_destroy(struct bt_buf *buf);

/* Storage for fragments (views) into the upper layers' PDUs. */
/* TODO: remove user-data requirements */
BT_BUF_POOL_FIXED_DEFINE(fragments, CONFIG_BT_CONN_FRAG_COUNT, 0,
...
static struct frag_md *get_frag_md(struct bt_buf *fragment)
{
	return &frag_md_pool[bt_buf_id(fragment)];
}

static void frag_destroy(struct bt_buf *frag)
{
...
static struct bt_buf *get_data_frag(struct bt_buf *outside, size_t winsize)
{
	struct bt_buf *window;

	__ASSERT_NO_MSG(!bt_buf_has_view(outside));

	/* Keeping a ref is the caller's responsibility */
	window = bt_buf_alloc_len(&fragments, 0, OS_TIMEOUT_NO_WAIT);
	if (!window) {
		return window;
	}
...
os_sem_t *bt_conn_get_pkts(struct bt_conn *conn)
{
#if defined(CONFIG_BT_CLASSIC)
...
static void tx_free(struct bt_conn_tx *tx)
{
	...
	tx->cb = NULL;
	tx->user_data = NULL;
	bt_fifo_put(&free_tx, tx);
}

#if defined(CONFIG_BT_CONN_TX)
static struct bt_work_q *tx_notify_workqueue_get(void)
{
#if defined(CONFIG_BT_CONN_TX_NOTIFY_WQ)
...
static void tx_notify_process(struct bt_conn *conn)
{
	...
	__ASSERT_NO_MSG(os_thread_self() == bt_work_queue_thread_get(tx_notify_workqueue_get()));
...
void bt_conn_tx_notify(struct bt_conn *conn, bool wait_for_completion)
{
	...
	if (os_thread_self() == bt_work_queue_thread_get(tx_notify_workqueue_get())) {
...
#if defined(CONFIG_BT_CONN)
	bt_work_init_delayable(&conn->deferred_work, deferred_work);
#endif /* CONFIG_BT_CONN */
#if defined(CONFIG_BT_CONN_TX)
	bt_work_init(&conn->tx_complete_work, tx_complete_work);
#endif /* CONFIG_BT_CONN_TX */
...
void bt_conn_reset_rx_state(struct bt_conn *conn)
{
	...
	bt_buf_unref(conn->rx);
	conn->rx = NULL;
}

static void bt_acl_recv(struct bt_conn *conn, struct bt_buf *buf,
			uint8_t flags)
{
	...
	case BT_ACL_CONT:
		...
		bt_buf_add_mem(conn->rx, buf->data, buf->len);
		bt_buf_unref(buf);
		break;
	default:
...
{
	return bt_fifo_is_empty(&free_tx);
}

static struct bt_conn_tx *conn_tx_alloc(void)
{
	struct bt_conn_tx *ret = bt_fifo_get(&free_tx, OS_TIMEOUT_NO_WAIT);
...
static int send_acl(struct bt_conn *conn, struct bt_buf *buf, uint8_t flags)
{
	...
	bt_buf_push_u8(buf, BT_HCI_H4_ACL);

	return bt_send(buf);
}

static enum bt_iso_timestamp contains_iso_timestamp(struct bt_buf *buf)
{
	enum bt_iso_timestamp ts;

	if (bt_buf_headroom(buf) ==
...
static int send_iso(struct bt_conn *conn, struct bt_buf *buf, uint8_t flags)
{
...
static int send_buf(struct bt_conn *conn, struct bt_buf *buf,
		    size_t len, bt_conn_tx_cb_t cb, void *ud)
{
	...
	if (os_sem_take(bt_conn_get_pkts(conn), OS_TIMEOUT_NO_WAIT)) {
	...
	if (!tx) {
		...
		os_sem_give(bt_conn_get_pkts(conn));
		err = -ENOMEM;
		goto error_return;
	}
	 ...
	 * os_thread_yield() before returning.
	 *
...
static struct bt_poll_signal conn_change =
		BT_POLL_SIGNAL_INITIALIZER(conn_change);
...
{
	return bt_slist_peek_head(&conn->l2cap_data_ready) != NULL;
}
...
void bt_conn_data_ready(struct bt_conn *conn)
{
	...
	if (!bt_slist_find(&bt_dev.le.conn_ready, &conn->_conn_ready, NULL)) {
		bt_slist_append(&bt_dev.le.conn_ready, &conn->_conn_ready);
...
{
	return os_sem_count_get(bt_conn_get_pkts(conn)) == 0;
}
...
static struct bt_conn *get_conn_ready(void)
{
	...
	BT_SLIST_FOR_EACH_CONTAINER_SAFE(&bt_dev.le.conn_ready, conn, tmp, _conn_ready) {
		...
		if (should_stop_tx(conn)) {
			...
			bt_slist_remove(&bt_dev.le.conn_ready, prev, &conn->_conn_ready);
...
#if defined(CONFIG_BT_CONN)
static void acl_get_and_clear_cb(struct bt_conn *conn, struct bt_buf *buf,
...
static void process_unack_tx(struct bt_conn *conn)
{
	...
	while (1) {
		...
		node = bt_slist_get(&conn->tx_pending);
...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
	...
	case BT_CONN_INITIATING:
		if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
			...
			bt_work_cancel_delayable(&conn->deferred_work);
		}
		break;
	default:
		break;
	...
	case BT_CONN_CONNECTED:
		...
		bt_poll_signal_raise(&conn_change, 0);
	...
	case BT_CONN_DISCONNECTED:
		...
		case BT_CONN_DISCONNECT_COMPLETE:
			...
			bt_work_reschedule(&conn->deferred_work, OS_TIMEOUT_NO_WAIT);
	...
	case BT_CONN_INITIATING:
		...
		if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
			...
			bt_work_schedule(&conn->deferred_work,
					OS_MSEC(10 * bt_dev.create_param.timeout));
		}
...
static OS_SEM_DEFINE(pending_recycled_events, 0, OS_SEM_MAX_LIMIT);

static void recycled_work_handler(struct bt_work *work)
{
	if (os_sem_take(&pending_recycled_events, OS_TIMEOUT_NO_WAIT) == 0) {
		notify_recycled_conn_slot();
		bt_work_submit(work);
	}
}

static BT_WORK_DEFINE(recycled_work, recycled_work_handler);
...
void bt_conn_unref(struct bt_conn *conn)
{
	 ...
	 */
	os_sem_give(&pending_recycled_events);
	bt_work_submit(&recycled_work);
}
...
uint8_t bt_conn_index(const struct bt_conn *conn)
{
	...
	case BT_CONN_TYPE_ISO:
		...
		__ASSERT_MSG(index >= 0 && index < ARRAY_SIZE(iso_conns),
	...
	case BT_CONN_TYPE_SCO:
		...
		__ASSERT_MSG(index >= 0 && index < ARRAY_SIZE(sco_conns),
			"Invalid bt_conn pointer");
		break;
#endif
	default:
		...
		__ASSERT_MSG(index >= 0 && index < ARRAY_SIZE(acl_conns),
			 "Invalid bt_conn pointer");
#else
		__ASSERT_MSG(false, "Invalid connection type %u", conn->type);
#endif /* CONFIG_BT_CONN */
...
struct bt_buf *bt_conn_create_pdu_timeout_debug(struct bt_buf_pool *pool,
						 size_t reserve,
						 os_timeout_t timeout,
						 const char *func, int line)
#else
struct bt_buf *bt_conn_create_pdu_timeout(struct bt_buf_pool *pool,
					   size_t reserve, os_timeout_t timeout)
#endif
{
	struct bt_buf *buf;

	/*
	 * PDU must not be allocated from ISR as we block with 'OS_TIMEOUT_FOREVER'
	 * during the allocation
	 */
	//__ASSERT_NO_MSG(!k_is_in_isr());

	if (!TIMEOUT_EQ(timeout, OS_TIMEOUT_NO_WAIT) &&
	    os_thread_self() == bt_work_queue_thread_get(&main_work_q)) {
...
#if defined(CONFIG_BT_BUF_LOG)
		buf = bt_buf_alloc_fixed_debug(pool, OS_TIMEOUT_NO_WAIT, func, line);
#else
		buf = bt_buf_alloc(pool, OS_TIMEOUT_NO_WAIT);
#endif
		if (!buf) {
			LOG_WRN("Unable to allocate buffer with OS_TIMEOUT_NO_WAIT");
#if defined(CONFIG_BT_BUF_LOG)
			buf = bt_buf_alloc_fixed_debug(pool, timeout, func,
							line);
#else
			buf = bt_buf_alloc(pool, timeout);
#endif
		}
	} else {
#if defined(CONFIG_BT_BUF_LOG)
 ...
 * - update data length
 */
static void auto_initiated_procedures(struct bt_work *unused)
{
	ARG_UNUSED(unused);
...
static BT_WORK_DEFINE(procedures_on_connect, auto_initiated_procedures);
...
static void deferred_work(struct bt_work *work)
{
	...
	if (IS_ENABLED(CONFIG_BT_CENTRAL) &&
		...
		if (bt_le_create_conn_cancel() == -ENOBUFS) {
			...
			bt_work_reschedule(dwork, OS_MSEC(10));
		}
		return;
	}
...
{
	if (bt_slist_find(&conn_cbs, &cb->_node, NULL)) {
		return -EEXIST;
	}

	bt_slist_append(&conn_cbs, &cb->_node);
...
int bt_conn_cb_unregister(struct bt_conn_cb *cb)
{
	...
	if (!bt_slist_find_and_remove(&conn_cbs, &cb->_node)) {
		return -ENOENT;
	}
						...
						struct bt_conn_le_frame_space_updated *params)
{
	if (IS_ENABLED(CONFIG_BT_CONN_DYNAMIC_CALLBACKS)) {
		...
		BT_SLIST_FOR_EACH_CONTAINER(&conn_cbs, callback, _node) {
...
int bt_conn_auth_info_cb_register(struct bt_conn_auth_info_cb *cb)
{
	...
	if (bt_slist_find(&bt_auth_info_cbs, &cb->node, NULL)) {
		return -EALREADY;
	}

	bt_slist_append(&bt_auth_info_cbs, &cb->node);
...
int bt_conn_auth_info_cb_unregister(struct bt_conn_auth_info_cb *cb)
{
	...
	if (!bt_slist_find_and_remove(&bt_auth_info_cbs, &cb->node)) {
		return -EALREADY;
	}
...
void bt_hci_le_df_connection_iq_report_common(uint8_t event, struct bt_buf *buf)
{
...
void bt_hci_le_vs_df_connection_iq_report(struct bt_buf *buf)
{
...
void bt_hci_le_df_cte_req_failed(struct bt_buf *buf)
{
	struct bt_df_conn_iq_samples_report iq_report;
	struct bt_conn *conn;
	int err;
...
static int bt_conn_tx_workq_init(void)
{
	...
	bt_work_queue_init(&conn_tx_workq);
	bt_work_queue_start(&conn_tx_workq, CONFIG_BT_CONN_TX_NOTIFY_WQ_STACK_SIZE,
			   OS_PRIORITY(CONFIG_BT_CONN_TX_NOTIFY_WQ_PRIO), &cfg);

	return 0;
}

STACK_INIT(bt_conn_tx_workq_init, STACK_RUN_INIT, CONFIG_BT_CONN_TX_NOTIFY_WQ_INIT_PRIORITY);
#endif /* CONFIG_BT_CONN_TX_NOTIFY_WQ */
