...
static struct bt_buf *get_rx(const uint8_t *buf)
{
	...
	os_timeout_t timeout = OS_TIMEOUT_FOREVER;
	...
	case BT_HCI_H4_ACL:
		return bt_buf_get_rx(BT_BUF_ACL_IN, OS_TIMEOUT_FOREVER);
	case BT_HCI_H4_ISO:
		if (IS_ENABLED(CONFIG_BT_ISO)) {
			return bt_buf_get_rx(BT_BUF_ISO_IN, OS_TIMEOUT_FOREVER);
		}
		__fallthrough;
	default:
...
static void h4_rx_thread(void *p1)
{
	...
	while (1) {
		...
		while (frame_size > 0) {
			...
			h4->recv(transport, buf);
			os_thread_yield();
		}
	}
}

static int h4_send(const struct bt_hci_transport *transport, struct bt_buf *buf)
{
	struct h4_data *h4 = transport->user_data;
	enum bt_buf_type type = bt_buf_type_from_h4(bt_buf_pull_u8(buf), BT_BUF_OUT);
	int len;
	int ret;
	...
	case BT_BUF_ACL_OUT:
		...
		break;
	case BT_BUF_CMD:
		bt_buf_push_u8(buf, BT_HCI_H4_CMD);
		break;
	case BT_BUF_ISO_OUT:
		if (IS_ENABLED(CONFIG_BT_ISO)) {
	...
	bt_buf_unref(buf);
	os_mutex_unlock(&h4->mutex);

	return ret < 0 ? ret : 0;
}

static int h4_open(const struct bt_hci_transport *transport, bt_hci_recv_t recv)
{
	int ret;
	struct h4_data *h4 = transport->user_data;
